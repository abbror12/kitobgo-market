      ...
      <section className="container-page pb-7 sm:pb-12">
        <div className="grid gap-4 rounded-[18px] border border-line bg-cream p-3 shadow-soft sm:gap-8 sm:rounded-[24px] sm:p-7 lg:grid-cols-[.88fr_1.12fr] lg:gap-12 lg:p-10">
          ...
          <div className="flex flex-col justify-center">
            ...
            <div className="mt-4 flex items-end gap-3 sm:mt-6"><span className="font-serif text-2xl font-semibold text-brand sm:text-3xl">{formatPrice(book.price)}</span>{book.oldPrice && <span className="pb-1 text-sm text-bodyText line-through">{formatPrice(book.oldPrice)}</span>}</div>
            <ProductActions book={book} />
            <div className="mt-4 grid grid-cols-2 gap-2 border-t border-line pt-4 sm:mt-7 sm:gap-3 sm:pt-6">
              <span className="flex items-center gap-2 text-[11px] sm:gap-3 sm:text-sm"><Truck size={18} className="shrink-0 text-brand sm:size-5" /><span><strong className="block">Bepul yetkazish</strong><small className="hidden text-bodyText sm:inline">O‘zbekiston bo‘ylab</small></span></span>
              <span className="flex items-center gap-2 text-[11px] sm:gap-3 sm:text-sm"><CreditCard size={18} className="shrink-0 text-brand sm:size-5" /><span><strong className="block">Qabul qilganda to‘lov</strong><small className="hidden text-bodyText sm:inline">Avval tekshiring</small></span></span>
            </div>
          </div>
        </div>
      </section>
