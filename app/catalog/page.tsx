...
export default async function CatalogPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return (
    <StoreShell>
      <PageIntro eyebrow="Onlayn kutubxona" title={selectedName ?? (query ? `“${query}” bo‘yicha natijalar` : "Kitoblar katalogi")} description="Didingiz va ehtiyojingizga mos kitobni qulay filtrlar yordamida toping." breadcrumbs={[{ label: "Katalog" }]} aside={<span className="rounded-full bg-sand px-4 py-2 text-sm font-bold text-brand">{result.totalElements} ta kitob</span>} />
      <section className="container-page py-5 sm:py-12">
        {rootCategories.length > 0 && <div id="kategoriyalar" className="scrollbar-hide -mx-3 flex gap-2 overflow-x-auto px-3 pb-4 sm:mx-0 sm:flex-wrap sm:px-0 sm:pb-6">
          <Link href="/catalog" className={`shrink-0 rounded-full border px-4 py-2 text-sm font-semibold transition ${!selectedCategory ? "border-brand bg-brand text-cream" : "border-line bg-cream text-ink hover:border-brand/30"}`}>Barchasi</Link>
          {rootCategories.map((category) => <Link key={category.id} href={`/catalog?category=${category.id}`} className={`shrink-0 rounded-full border px-4 py-2 text-sm font-semibold transition ${selectedCategory === String(category.id) ? "border-brand bg-brand text-cream" : "border-line bg-cream text-ink hover:border-brand/30"}`}>{category.name}</Link>)}
        </div>}
        <div className="grid gap-4 lg:grid-cols-[240px_1fr] lg:gap-7 xl:grid-cols-[260px_1fr]">
          <aside>
            ...
            <form action="/catalog" className="mt-2 hidden rounded-2xl border border-line bg-cream p-4 group-open:block lg:sticky lg:top-40 lg:mt-0 lg:block lg:p-5">
              <div className="flex items-center gap-2 border-b border-line pb-4"><SlidersHorizontal size={18} className="text-brand" /><h2 className="font-serif font-semibold">Filtrlar</h2></div>
              ...
              <select id="category" name="category" defaultValue={selectedCategory} className="mt-2 h-11 w-full rounded-xl border border-line bg-cream px-3 text-sm outline-none focus:border-brand">
              ...
              <select id="max" name="max" defaultValue={params.max ?? ""} className="mt-2 h-11 w-full rounded-xl border border-line bg-cream px-3 text-sm outline-none focus:border-brand">
              ...
              <select id="stock" name="stock" defaultValue={params.stock ?? ""} className="mt-2 h-11 w-full rounded-xl border border-line bg-cream px-3 text-sm outline-none focus:border-brand">
              ...
              <select id="discount" name="discount" defaultValue={params.discount ?? ""} className="mt-2 h-11 w-full rounded-xl border border-line bg-cream px-3 text-sm outline-none focus:border-brand">
              ...
              {hasFilters && <Link href="/catalog" className="mt-3 flex items-center justify-center gap-1.5 text-sm font-semibold text-bodyText hover:text-brand"><X size={15} /> Tozalash</Link>}
            </form>
            </details>
          </aside>
          <div>
            <div className="mb-5 flex items-center justify-between gap-3">
              ...
              <form action="/catalog">
                ...
                <select name="sort" defaultValue={selectedSort} className="h-10 rounded-xl border border-line bg-cream px-3 text-sm font-medium outline-none focus:border-brand" aria-label="Saralash">
                ...
                <button type="submit" className="ml-2 h-10 rounded-xl border border-line bg-cream px-3 text-xs font-bold hover:border-brand hover:text-brand">Saralash</button>
              </form>
            </div>
            {result.content.length ? <>
              ...
              {result.totalPages > 1 && <nav className="mt-8 flex items-center justify-center gap-3" aria-label="Katalog sahifalari">
                ...
                {!result.last ? <Link href={catalogHref(params, currentPage + 1)} className="inline-flex h-10 items-center gap-1 rounded-xl border border-line bg-cream px-4 text-sm font-bold hover:border-brand hover:text-brand">Keyingi <ChevronRight size={17} /></Link> : <span className="inline-flex h-10 cursor-not-allowed items-center gap-1 rounded-xl border border-line bg-cream px-4 text-sm font-bold text-bodyText opacity-50">Keyingi <ChevronRight size={17} /></span>}
              </nav>}
