  ...
  return (
    <div className="mt-4 sm:mt-6">
      <div className="flex gap-2 sm:gap-3">
        <div className="flex h-11 shrink-0 items-center rounded-xl border border-line bg-white p-0.5 sm:h-12 sm:p-1" aria-label="Miqdor">
          ...
          <button type="button" onClick={() => setQuantity((value) => Math.min(maxQuantity, value + 1))} className="grid size-9 place-items-center rounded-lg transition hover:bg-canvas" aria-label="Ko‘paytirish"><Plus size={16} /></button>
        </div>
        {book.inStock ? <Link href={`/checkout?book=${book.slug}&quantity=${quantity}`} className="button-primary h-11 flex-1 px-3 text-sm sm:h-12 sm:px-5 sm:text-base"><ShoppingBag size={18} /> Hoziroq xarid qilish</Link> : <button type="button" disabled className="h-11 flex-1 rounded-xl bg-black/10 px-3 text-sm font-bold text-muted sm:h-12 sm:px-5">Hozircha mavjud emas</button>}
      </div>
