...
export function TestimonialCard({ testimonial }: { testimonial: Testimonial }) {
  return (
    <article className="rounded-2xl border border-line bg-cream p-4 sm:p-6">
      <div className="flex items-center justify-between"><div className="flex gap-0.5" aria-label={`${testimonial.rating} yulduz`}>{Array.from({ length: testimonial.rating }, (_, index) => <Star key={index} size={16} className="fill-gold text-gold" aria-hidden="true" />)}</div><Quote className="text-cocoa/15" size={30} aria-hidden="true" /></div>
