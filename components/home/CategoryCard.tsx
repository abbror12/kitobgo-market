...
export function CategoryCard({ category }: { category: Category }) {
  ...
  return (
    <Link href={`/catalog?category=${category.id}`} className="group flex w-[112px] shrink-0 flex-col items-center justify-center rounded-2xl border border-line bg-cream px-2 py-4 text-center transition hover:-translate-y-1 hover:border-brand/25 hover:shadow-card sm:w-auto">
      <span className="grid size-12 place-items-center rounded-2xl bg-sand text-brand transition group-hover:bg-brand group-hover:text-cream"><Icon size={25} strokeWidth={1.6} aria-hidden={true} /></span>
