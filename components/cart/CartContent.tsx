      ...
      <div className="space-y-3">
        {items.map(({ book, quantity }) => (
          <article key={book.id} className="flex gap-4 rounded-2xl border border-line bg-cream p-3 sm:items-center sm:p-4">
            ...
            <div className="min-w-0 flex-1 self-stretch py-1"><Link href={`/books/${book.slug}`} className="font-serif font-semibold leading-5 hover:text-brand sm:text-lg">{book.title}</Link><p className="mt-1 line-clamp-1 text-xs text-bodyText sm:text-sm">{book.author}</p><p className="mt-3 font-serif font-semibold text-brand sm:text-lg">{formatPrice(book.price)}</p><div className="mt-3 flex items-center justify-between sm:hidden"><Quantity value={quantity} onMinus={() => updateQuantity(book.id, -1)} onPlus={() => updateQuantity(book.id, 1)} /><button onClick={() => save(items.filter((item) => item.book.id !== book.id))} className="icon-button text-bodyText hover:text-danger" aria-label="Olib tashlash"><Trash2 size={18} /></button></div></div>
